...
def plan(self):
    ...
    if(self.action_state != 'catching'):
        self.action_state = 'catching'
        self.path = deepcopy(pathTypes.catch)
    '''elif(self.action_state != 'clearing' and dist(self.ball_loc, self.goal)>(dist(self.player_loc, self.goal)+100)):
        self.action_state = 'clearing'
        print( 'Switch to state ' + self.action_state )
        self.path = deepcopy(pathTypes.clear)'''
...
def Follow_Line(self):
    ...
    if(self.line.do_jump(dist2d(self.line.start, self.line.end)) and not self.is_kickoff):
        self.jump = True
        self.jumping = True
    else:
        self.jump = False
    ...
    forward = np.array([math.cos(self.pitch) * math.cos(self.yaw), math.cos(self.pitch) * math.sin(self.yaw), math.sin(self.pitch)])

    if self.flipping or self.line.events[0].flip:
        #self.flip = AirDodge(self.info.my_car)
        self.flipping = True
    else:
         self.flip_idx = 0