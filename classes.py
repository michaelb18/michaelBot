...
class path:
    ...
    def do_jump( self, distance ):
        return self.get_line().do_jump(distance)
...
class line:
    def __init__(self, start=ZEROS3, end=ZEROS3, speed=MAX_CAR_SPEED, jump = False, flip = False):
        self.start = start
        self.end = end
        self.arm = True
        self.finished = False
        self.events = [Event(0, speed, jump, flip, True)]  # lines will always have at least one speed event
...
class Event():
    def __init__(self, distance, speed=MAX_CAR_SPEED, jump = False, flip = False, anchored=False):
